...
# app.config['SQLALCHEMY_DATABSE_URI'] = 'sqlite:///' + os.path.join(basedir, 'planets.db')
...
app.config['MAIL_USERNAME'] = 'f39dfd634dd8c3'
app.config['MAIL_PASSWORD'] = 'a13dfee5aa711d'
...
def add_planet():
    ...
    # if test:
    #     return jsonify(message="This planet already exists")
    # else:
    ...
    db.session.commit()
    return jsonify(message="You created a new planet"), 201
...
def update_planet():
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message="Planet was updated")
    else:
...
def delete_planet(planet_id:int):
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message=f'Planet {planet.planet_name} was removed'), 202
    else: 
        return jsonify(message='That planet does not exist'), 404
